...
def load_data(path='data/', dataset='tagged', seed=80):
    ...
    if dataset=="cvpa":
        labels= torch.from_numpy(node_features['overdue7'].to_numpy()).type(torch.LongTensor)  # label tensor
        sensitive_attribute = torch.from_numpy(node_features['sex'].to_numpy()).type(torch.LongTensor)
        node_features=node_features.drop(["overdue7","sex"],axis=1)
        features = node_features[node_features.columns].to_numpy()
    else:
